...
export const LAB_SYMBOL_MODULES = 41;
export const LAB_ACTIVE_QUIET_MODULES = 2;
...
export const LAB_FRAME = {
  ...
  symbolSize: 656,
  modulePitch: 16,
  moduleCount: LAB_SYMBOL_MODULES,
  quietModules: LAB_ACTIVE_QUIET_MODULES,
  tileSize: 8,
} as const;

export const LAB_PROFILE_NAME = "PRISM-C16-CUSTOM-R1";
export const LAB_BOOTSTRAP_PATH = "/o";
export const LAB_TARGET_FPS = 10;
export const LAB_GEOMETRY_TRACK_MAX_FRAMES = 2;
export const LAB_GEOMETRY_TRACK_MAX_AGE_MS = 260;
export const LAB_PALETTE_SIZE = 16;
export const LAB_CHROMA_RADIX = 16;
export const LAB_PACKET_BYTES = 516;
export const LAB_INNER_STRIPE_DATA_BYTES = 172;
export const LAB_INNER_PARITY_BYTES = 33;
export const LAB_INNER_STRIPE_CODEWORD_BYTES = LAB_INNER_STRIPE_DATA_BYTES + LAB_INNER_PARITY_BYTES;
export const LAB_INNER_STRIPE_COUNT = 3;
export const LAB_INNER_CODEWORD_BYTES = LAB_INNER_STRIPE_CODEWORD_BYTES * LAB_INNER_STRIPE_COUNT;
export const LAB_INNER_TOTAL_PARITY_BYTES = LAB_INNER_PARITY_BYTES * LAB_INNER_STRIPE_COUNT;
export const LAB_INNER_CODE_RATE = LAB_PACKET_BYTES / LAB_INNER_CODEWORD_BYTES;
export const LAB_SOURCE_CHUNK_BYTES = 480;
export const LAB_SOURCE_CHUNK_COUNT = 8;
export const LAB_OBJECT_BYTES = LAB_SOURCE_CHUNK_BYTES * LAB_SOURCE_CHUNK_COUNT;
export const LAB_SYMBOLS_PER_CODEWORD = LAB_INNER_CODEWORD_BYTES * 2;
...
export const LAB_PROFILE_AREA_GAIN = (LAB_PREVIOUS_PROFILE_TOTAL_MODULES / (
  LAB_SYMBOL_MODULES + (2 * LAB_ACTIVE_QUIET_MODULES)
)) ** 2;
export const LAB_VERIFIED_PAYLOAD_DENSITY_GAIN = (
  LAB_SOURCE_CHUNK_BYTES / ((LAB_SYMBOL_MODULES + (2 * LAB_ACTIVE_QUIET_MODULES)) ** 2)
) / (
  LAB_PREVIOUS_SOURCE_CHUNK_BYTES / (LAB_PREVIOUS_PROFILE_TOTAL_MODULES ** 2)
);
export const LAB_RX5_PAYLOAD_GAIN = LAB_SOURCE_CHUNK_BYTES / 174;

// CIE-Lab-separated camera palette. Black and white are payload symbols, not
// merely geometry colors. The remaining states span hue and luminance so the
// decoder can estimate a full camera-space constellation on every frame.
export const C16_PRISM_PALETTE = [
  [0, 0, 0],
  [128, 0, 0],
  [230, 35, 35],
  [245, 125, 20],
...
const PACKET_MAGIC = new Uint8Array([0x50, 0x52, 0x53, 0x4d]);
const OBJECT_MAGIC = new Uint8Array([0x50, 0x47, 0x44, 0x4f]);
const FRAME_VERSION = 6;
const C16_CUSTOM_PROFILE_CODE = 11;
...
const OBJECT_MESSAGE = new TextEncoder().encode("PRISM-C16-CUSTOM-PHY-OK");
const GLOBAL_PILOT_REPEATS = 4;
const FINDER_SIZE = 7;
const FINDER_DISTANCE = LAB_SYMBOL_MODULES - FINDER_SIZE;
const WHITE_STATE = LAB_PALETTE_SIZE - 1;
let MATRIX_CACHE: PrismMatrix | null = null;

export type Point = Readonly<{ x: number; y: number }>;
export type Rgb = Readonly<[number, number, number]>;

export type PrismCell = Readonly<{
  index: number;
  row: number;
  column: number;
  tileRow: number;
...
export type PrismPilot = Readonly<PrismCell & {
  paletteState: number;
  scope: "global" | "local";
}>;

export type PrismMatrix = Readonly<{
  size: number;
  functionStates: Uint8Array;
  reserved: Uint8Array;
  pilots: readonly PrismPilot[];
  payloadCells: readonly PrismCell[];
}>;
...
export type FrameGridDecode = Readonly<{
  frame: DynamicLabFrame;
  repairMode: "triple-cauchy-mds-erasure";
  correctedByteErasures: number;
  reliabilityErasedBytes: number;
  inferredMaskId: number;
}>;

export type BootstrapControl = Readonly<{ profile: typeof LAB_PROFILE_NAME }>;
...
export const PRISM_FINDER_CENTERS: readonly Point[] = [
  { x: 3.5, y: 3.5 },
  { x: 3.5 + FINDER_DISTANCE, y: 3.5 },
  { x: 3.5 + FINDER_DISTANCE, y: 3.5 + FINDER_DISTANCE },
  { x: 3.5, y: 3.5 + FINDER_DISTANCE },
];
...
export function buildLabObject(sessionNonce: Uint8Array): DynamicLabObject {
  ...
  bytes[6] = C16_CUSTOM_PROFILE_CODE;
...
export function prepareDynamicFrame(
  ...
  for (let maskId = 0; maskId < 16; maskId += 1) {
    ...
    const symbols = buildUnmaskedPayloadSymbols(frame, matrix.payloadCells.length);
    const masked = applyPaletteMask(symbols, maskId, matrix.payloadCells);
    const paletteStates = composePalette(matrix, masked);
    const maskScore = scorePaletteStates(matrix, paletteStates, previousPaletteStates);
    if (!selected || maskScore > selected.maskScore) selected = { frame, matrix, paletteStates, maskScore };
  }
  if (!selected) throw new Error("no custom color mask could be selected");
  return selected;
}
...
export function buildDynamicFrame(
  ...
  bytes[7] = C16_CUSTOM_PROFILE_CODE;
  bytes.set(sessionNonce, 8);
...
export function createPrismMatrix(): PrismMatrix {
  if (MATRIX_CACHE) return MATRIX_CACHE;
  const size = LAB_SYMBOL_MODULES;
  const functionStates = new Uint8Array(size * size).fill(WHITE_STATE);
  const reserved = new Uint8Array(size * size);
  placeFinder(functionStates, reserved, 0, 0, size);
  placeFinder(functionStates, reserved, 0, size - FINDER_SIZE, size);
  placeFinder(functionStates, reserved, size - FINDER_SIZE, size - FINDER_SIZE, size);
  placeFinder(functionStates, reserved, size - FINDER_SIZE, 0, size);
  for (let column = 8; column <= size - 9; column += 1) {
    setFunction(functionStates, reserved, 8, column, column % 2 === 0 ? 0 : WHITE_STATE, size);
  }
  for (let row = 9; row <= size - 9; row += 1) {
    setFunction(functionStates, reserved, row, 8, row % 2 === 0 ? 0 : WHITE_STATE, size);
  }
  const orientation = [
    [1, 1, 1, 0, 1],
    [1, 0, 0, 0, 1],
  ...
  orientation.forEach((line, rowOffset) => line.forEach((dark, columnOffset) => {
    setFunction(
      functionStates,
      reserved,
      10 + rowOffset,
  ...
  const pilots = selectPilots(reserved, size);
  const pilotIndexes = new Set(pilots.map((pilot) => pilot.index));
  const payloadCells: PrismCell[] = [];
  for (let row = 0; row < size; row += 1) {
    for (let column = 0; column < size; column += 1) {
      const cell = createCell(row, column, size);
      if (!reserved[cell.index] && !pilotIndexes.has(cell.index)) payloadCells.push(cell);
    }
  }
  payloadCells.sort((left, right) => cellInterleaveKey(left) - cellInterleaveKey(right) || left.index - right.index);
  if (payloadCells.length < LAB_SYMBOLS_PER_CODEWORD) throw new Error("custom grid does not have enough payload modules");
  MATRIX_CACHE = { size, functionStates, reserved, pilots, payloadCells };
  return MATRIX_CACHE;
}

export function encodeBytesToNibbles(bytes: Uint8Array): number[] {
  const symbols: number[] = [];
  for (const byte of bytes) symbols.push((byte >>> 4) & 0x0f, byte & 0x0f);
  return symbols;
}

export function decodeNibblesToOptionalBytes(symbols: readonly (number | null)[]): Array<number | null> {
  if (symbols.length % 2 !== 0) throw new Error("nibble symbol count must be even");
  const bytes: Array<number | null> = [];
  for (let offset = 0; offset < symbols.length; offset += 2) {
    const high = symbols[offset];
    const low = symbols[offset + 1];
    if (high === null || low === null) {
      bytes.push(null);
    } else if (!Number.isInteger(high) || !Number.isInteger(low) || high < 0 || high > 15 || low < 0 || low > 15) {
      throw new Error("symbol is outside the 16-color alphabet");
    } else {
...
export function applyPaletteMask(symbols: readonly number[], maskId: number, cells: readonly PrismCell[]): number[] {
  validateMaskId(maskId);
  if (symbols.length > cells.length) throw new Error("palette symbols exceed custom payload cells");
  return symbols.map((symbol, index) => modulo(symbol + maskDelta(maskId, cells[index]), LAB_CHROMA_RADIX));
}

export function removePaletteMask(
  symbols: readonly (number | null)[],
  maskId: number,
  cells: readonly PrismCell[],
): Array<number | null> {
  validateMaskId(maskId);
  if (symbols.length > cells.length) throw new Error("palette symbols exceed custom payload cells");
  return symbols.map((symbol, index) => symbol === null
    ? null
    : modulo(symbol - maskDelta(maskId, cells[index]), LAB_CHROMA_RADIX));
}

export function decodePrismPaletteSymbols(
  ...
  if (observedPaletteStates.length < LAB_SYMBOLS_PER_CODEWORD) {
    throw new Error("not enough custom color symbols for the triple inner codeword");
  }
  const constrained = observedPaletteStates.slice(0, LAB_SYMBOLS_PER_CODEWORD).map((state) => (
    state === null || !Number.isInteger(state) || state < 0 || state >= LAB_PALETTE_SIZE ? null : state
  ));
  const maskIds = knownMaskId === undefined ? Array.from({ length: 16 }, (_, index) => index) : [knownMaskId];
  const candidates = maskIds.map((maskId) => {
    const unmasked = removePaletteMask(constrained, maskId, matrix.payloadCells);
    return { maskId, codeword: decodeNibblesToOptionalBytes(unmasked) };
  });
  const valid: FrameGridDecode[] = [];
  for (const candidate of candidates) {
    const orders = buildStripeReliabilityOrders(symbolReliabilities, candidate.codeword);
    const baseErasures = Array.from({ length: LAB_INNER_STRIPE_COUNT }, (_, stripe) => (
      candidate.codeword
        .slice(stripe * LAB_INNER_STRIPE_CODEWORD_BYTES, (stripe + 1) * LAB_INNER_STRIPE_CODEWORD_BYTES)
        .filter((value) => value === null).length
    ));
    const budget = Math.min(...baseErasures.map((count, stripe) => (
      Math.max(0, Math.min(LAB_INNER_PARITY_BYTES - count, orders[stripe].length))
    )));
    for (const chaseCount of reliabilityChaseCounts(budget)) {
      try {
        const packet = new Uint8Array(LAB_PACKET_BYTES);
        let corrected = 0;
        let reliabilityErased = 0;
        for (let stripe = 0; stripe < LAB_INNER_STRIPE_COUNT; stripe += 1) {
          const start = stripe * LAB_INNER_STRIPE_CODEWORD_BYTES;
          const codeword = candidate.codeword.slice(start, start + LAB_INNER_STRIPE_CODEWORD_BYTES);
          const count = Math.min(chaseCount, orders[stripe].length);
          for (let index = 0; index < count; index += 1) codeword[orders[stripe][index]] = null;
          const decoded = decodeCauchyMds(codeword, LAB_INNER_STRIPE_DATA_BYTES);
          packet.set(decoded.data, stripe * LAB_INNER_STRIPE_DATA_BYTES);
          corrected += decoded.recoveredDataErasures;
          reliabilityErased += count;
        }
        const frame = parseDynamicFrame(packet);
        if (frame.maskId !== candidate.maskId) continue;
        valid.push({
          frame,
          repairMode: "triple-cauchy-mds-erasure",
          correctedByteErasures: corrected,
          reliabilityErasedBytes: reliabilityErased,
          inferredMaskId: candidate.maskId,
        });
        break;
      } catch {
        // Try a deeper reliability chase or another reversible palette mask.
      }
    }
  }
  if (valid.length === 0) throw new Error("no palette mask produced valid triple MDS stripes and CRC32");
  ...
  ))) throw new Error("multiple valid palette masks disagree");
  return decoded;
}

export function moduleCenter(cell: Pick<PrismCell, "row" | "column">): Point {
  return {
    x: LAB_FRAME.symbolX + ((cell.column + 0.5) * LAB_FRAME.modulePitch),
    y: LAB_FRAME.symbolY + ((cell.row + 0.5) * LAB_FRAME.modulePitch),
  };
}

export function logicalModuleCenter(cell: Pick<PrismCell, "row" | "column">): Point {
...
export class DynamicLabDecoder {
  ...
  addFrame(frame: DynamicLabFrame): boolean {
    ...
    if (frame.sessionHex !== this.sessionHex || frame.objectLength !== this.objectLength || frame.objectCrc !== this.objectCrc) {
      throw new Error("outer symbol belongs to a different object");
    }
    ...
    // Eliminate every pivot already present, including pivots above the first
    // missing column. Stopping at the first missing pivot leaves later pivot
    // bits in the row and falsely reports full rank before reaching RREF.
    for (let existingPivot = 0; existingPivot < LAB_SOURCE_CHUNK_COUNT; existingPivot += 1) {
      const existing = this.rows.get(existingPivot);
      if (!existing || (mask & (1 << existingPivot)) === 0) continue;
      mask ^= existing.mask;
      xorInto(data, existing.data);
    }
    if (mask === 0) return false;
    const pivot = lowestSetBit(mask);
    for (const [existingPivot, existing] of Array.from(this.rows.entries())) {
      if ((existing.mask & (1 << pivot)) === 0) continue;
      const reduced = Uint8Array.from(existing.data);
      xorInto(reduced, data);
      this.rows.set(existingPivot, { mask: existing.mask ^ mask, data: reduced });
    }
...
export function estimatePalette(samples: readonly (readonly Rgb[])[]): PaletteModel {
  if (samples.length !== LAB_PALETTE_SIZE || samples.some((group) => group.length === 0)) {
    throw new Error("all sixteen custom palette states need observed samples");
  }
  ...
  for (let first = 0; first < means.length; first += 1) {
    for (let second = first + 1; second < means.length; second += 1) {
      if (colorDistance(means[first], means[second]) < 18) {
        throw new Error("observed custom palette separation is too low");
      }
    }
  }
...
export function localizePalette(globalModel: PaletteModel, pilots: readonly Rgb[]): PaletteModel {
  if (pilots.length !== 2 || globalModel.means.length !== LAB_PALETTE_SIZE) {
    throw new Error("a custom tile needs black and white local anchor observations");
  }
  const black = pilots[0];
  const white = pilots[1];
  const gains = [0, 1, 2].map((channel) => {
    const globalSpan = globalModel.means[WHITE_STATE][channel] - globalModel.means[0][channel];
...
function placeFinder(states: Uint8Array, reserved: Uint8Array, top: number, left: number, size: number) {
  for (let row = Math.max(0, top - 1); row <= Math.min(size - 1, top + FINDER_SIZE); row += 1) {
    for (let column = Math.max(0, left - 1); column <= Math.min(size - 1, left + FINDER_SIZE); column += 1) {
      setFunction(states, reserved, row, column, WHITE_STATE, size);
    }
  }
  for (let row = 0; row < FINDER_SIZE; row += 1) {
    for (let column = 0; column < FINDER_SIZE; column += 1) {
      ...
      setFunction(states, reserved, top + row, left + column, dark ? 0 : WHITE_STATE, size);
    }
  }
}
...
function selectPilots(reserved: Uint8Array, size: number): PrismPilot[] {
  const pilots: PrismPilot[] = [];
  const selected = new Set<number>();
  const candidates: PrismCell[] = [];
  for (let row = 0; row < size; row += 1) {
    for (let column = 0; column < size; column += 1) {
      const cell = createCell(row, column, size);
  ...
  for (let state = 0; state < LAB_PALETTE_SIZE; state += 1) {
    for (let region = 0; region < GLOBAL_PILOT_REPEATS; region += 1) {
      const available = candidates.filter((cell) => !selected.has(cell.index) && globalPilotRegion(cell, size) === region)
        .sort((left, right) => (
          pilotSelectionKey(left, 1 + state * GLOBAL_PILOT_REPEATS + region)
          - pilotSelectionKey(right, 1 + state * GLOBAL_PILOT_REPEATS + region)
          || left.index - right.index
        ));
      if (available.length === 0) throw new Error("custom grid cannot place spatial global palette pilots");
      selected.add(available[0].index);
      pilots.push({ ...available[0], paletteState: state, scope: "global" });
    }
  }
  ...
  for (let tileRow = 0; tileRow < tileCount; tileRow += 1) {
    for (let tileColumn = 0; tileColumn < tileCount; tileColumn += 1) {
      ...
      )).sort((left, right) => pilotSelectionKey(left, 91) - pilotSelectionKey(right, 91) || left.index - right.index);
      if (available.length < 2) continue;
      selected.add(available[0].index);
      selected.add(available[1].index);
      pilots.push({ ...available[0], paletteState: 0, scope: "local" });
      pilots.push({ ...available[1], paletteState: WHITE_STATE, scope: "local" });
    }
  }
  const global = pilots.filter((pilot) => pilot.scope === "global");
  if (global.length !== LAB_PALETTE_SIZE * GLOBAL_PILOT_REPEATS) throw new Error("custom palette pilot set is incomplete");
  return pilots;
}

function composePalette(matrix: PrismMatrix, maskedSymbols: readonly number[]): Uint8Array {
  const states = Uint8Array.from(matrix.functionStates);
  for (const pilot of matrix.pilots) states[pilot.index] = pilot.paletteState;
  matrix.payloadCells.forEach((cell, index) => {
    states[cell.index] = maskedSymbols[index];
  });
  return states;
}

function buildUnmaskedPayloadSymbols(frame: DynamicLabFrame, capacity: number): number[] {
  const codeword = new Uint8Array(LAB_INNER_CODEWORD_BYTES);
  for (let stripe = 0; stripe < LAB_INNER_STRIPE_COUNT; stripe += 1) {
    const data = frame.bytes.subarray(
      stripe * LAB_INNER_STRIPE_DATA_BYTES,
      (stripe + 1) * LAB_INNER_STRIPE_DATA_BYTES,
    );
    codeword.set(encodeCauchyMds(data, LAB_INNER_PARITY_BYTES), stripe * LAB_INNER_STRIPE_CODEWORD_BYTES);
  }
  const symbols = encodeBytesToNibbles(codeword);
  let state = (seedFromBytes(frame.bytes.subarray(8, 16)) ^ frame.sequence ^ 0x639ac3d1) >>> 0;
  while (symbols.length < capacity) {
    state = xorshift32(state);
    symbols.push(state & 0x0f);
  }
  return symbols;
}

function scorePaletteStates(matrix: PrismMatrix, states: Uint8Array, previous: readonly number[] | null): number {
  let score = 0;
  for (let row = 0; row < matrix.size; row += 1) {
    for (let column = 0; column < matrix.size; column += 1) {
      const index = row * matrix.size + column;
      if (column + 1 < matrix.size) score += paletteScore(states[index], states[index + 1]);
      if (row + 1 < matrix.size) score += paletteScore(states[index], states[index + matrix.size]);
      if (previous && index < previous.length) score += 0.28 * paletteScore(states[index], previous[index]);
    }
  }
  const counts = new Array<number>(LAB_PALETTE_SIZE).fill(0);
  for (const cell of matrix.payloadCells) counts[states[cell.index]] += 1;
  const target = matrix.payloadCells.length / LAB_PALETTE_SIZE;
  for (const count of counts) score -= Math.abs(count - target) * 0.9;
  return score;
}

function paletteScore(first: number, second: number): number {
  if (first === second) return -150;
  return colorDistance(C16_PRISM_PALETTE[first], C16_PRISM_PALETTE[second]);
}
...
function createCell(row: number, column: number, size: number): PrismCell {
  return {
...
function pilotSelectionKey(cell: PrismCell, salt: number): number {
  return hash32(
...
function globalPilotRegion(cell: PrismCell, size: number): number {
...
function cellInterleaveKey(cell: PrismCell): number {
  return hash32(Math.imul(cell.index + 1, 0x9e3779b1));
}

function maskDelta(maskId: number, cell: PrismCell): number {
  const { row, column } = cell;
  switch (maskId) {
    case 0: return row + column;
    case 1: return (2 * row) + column;
    case 2: return row + (2 * column);
    case 3: return (3 * row) + column;
    case 4: return row + (3 * column);
    case 5: return (row * column) + row + column;
    case 6: return (5 * row) + (3 * column);
    case 7: return (3 * row) + (5 * column);
    case 8: return Math.floor(row / 2) + (7 * column);
    case 9: return (7 * row) + Math.floor(column / 2);
    case 10: return (row * column) + (5 * row) + column;
    case 11: return (row * column) + row + (5 * column);
    case 12: return ((row % 5) * 3) + column;
    case 13: return row + ((column % 5) * 3);
    case 14: return (row * row) + (3 * column);
    case 15: return (3 * row) + (column * column);
    default: throw new Error("mask id is outside the supported table");
  }
}

function buildStripeReliabilityOrders(
  symbolReliabilities: readonly number[] | undefined,
  codeword: readonly (number | null)[],
): number[][] {
  if (!symbolReliabilities) return Array.from({ length: LAB_INNER_STRIPE_COUNT }, () => []);
  if (symbolReliabilities.length < LAB_SYMBOLS_PER_CODEWORD) throw new Error("soft reliability vector is too short");
  return Array.from({ length: LAB_INNER_STRIPE_COUNT }, (_, stripe) => {
    const start = stripe * LAB_INNER_STRIPE_CODEWORD_BYTES;
    return Array.from({ length: LAB_INNER_STRIPE_CODEWORD_BYTES }, (_, localByte) => {
      const byteIndex = start + localByte;
      return {
        localByte,
        reliability: Math.min(symbolReliabilities[byteIndex * 2], symbolReliabilities[(byteIndex * 2) + 1]),
      };
...
function reliabilityChaseCounts(budget: number): number[] {
  ...
  return Array.from(new Set([0, 1, 2, 4, 8, 12, 20, 28, 36, budget].filter((count) => count <= budget)))
